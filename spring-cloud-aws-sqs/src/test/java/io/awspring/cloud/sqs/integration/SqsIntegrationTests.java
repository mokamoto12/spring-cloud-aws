...
class SqsIntegrationTests extends BaseSqsIntegrationTest {
	...
	void maxConcurrentMessages() {
		List<Message<String>> messages = IntStream.range(0, 20)
				...
				.map(payload -> MessageBuilder.withPayload(payload).build()).collect(Collectors.toList());
		sqsTemplate.send(MAX_CONCURRENT_MESSAGES_QUEUE_NAME, messages);
		logger.warn("Sent messages to queue {} with messages {}", MAX_CONCURRENT_MESSAGES_QUEUE_NAME, messages);
